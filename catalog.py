...
def _iter_items(obj: Any) -> Iterable[Dict[str, Any]]:
    """
    Handle tables that may be list[dict] or dict[id->dict].
    Uniformly yield dict items.
    """
    if obj is None:
        return []
    if isinstance(obj, dict):
        # May be {id: {...}} or already a single item
...
def normalize_catalog(
    ...
    def norm_one(x: Dict[str, Any], domain: Domain) -> Optional[ToolItem]:
        ...
        # cpl must not be empty
        if kind2 == "control_panel_cpl" and not value:
            return None
 
        # exe/msc must not be empty either
...
def build_embedding_text(item: ToolItem) -> str:
    """
    Document text for embedding-based retrieval.
    """
    return (
